...
cp /etc/katello/katello.yml ${KATELLO_HOME}/config/katello.yml
./update_katello_yml.rb ${KATELLO_HOME}/config/katello.yml
...
scl enable ruby193 "cd ${KATELLO_HOME} && bundle install"
cd ${KATELLO_HOME} && echo "yes" | scl enable ruby193 "./script/katello-reset-dbs development ."
echo "Katello devel enviroment is setup"
echo "Please run the below to bring up the Katello Development Server:"
echo " 1) cd ${KATELLO_HOME}"
